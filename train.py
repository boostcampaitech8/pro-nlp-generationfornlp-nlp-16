...
def main(cfg: DictConfig):
    ...
    print(
        f"Sample: {tokenizer.decode(train_dataset[0]['input_ids'], skip_special_tokens=True)}"
    )
    ...
    peft_config = get_peft_config(
        ...
        target_modules=OmegaConf.to_container(
            cfg.model.peft.target_modules, resolve=True
        ),
    ...
    sft_config = get_sft_config(
        ...
        report_to=cfg.training.get("report_to", None),  # wandb 설정 추가
        run_name=cfg.training.get("run_name", None),  # wandb 설정
    )
    ...
    if trainer.state.best_metric is not None:
        print(
            f"Best {cfg.training.metric_for_best_model}: {trainer.state.best_metric:.4f}"
        )
    ...
    if trainer.state.log_history:
        ...
        if eval_logs:
            ...
            print(
                f"Best Accuracy: {best_acc_log['eval_accuracy']:.4f} (epoch {best_acc_log['epoch']})"
            )
