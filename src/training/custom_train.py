...
class FocalLossTrainer(SFTTrainer):
    # [수정] 매 스텝마다 객체를 생성하면 느려지므로, __init__에서 한 번만 만듭니다.
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # 여기서 ignore_index=-100을 적용해 둡니다.
        self.loss_fct = FocalLoss(gamma=2.0, alpha=1.0, ignore_index=-100)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 2. Focal Loss 객체 생성 
        # (수정됨: 위 __init__에서 미리 만든 self.loss_fct를 사용하여 속도 최적화)
